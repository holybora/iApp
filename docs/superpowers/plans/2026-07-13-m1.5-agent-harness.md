...
- [ ] **Step 1: `app.config.ts` — DO NOT TOUCH.** Superseded by PO decision
during M1.5 execution (design spec §2): commit `5b06a75` set a REAL EAS
project ID, and the committed owner/slug/projectId stay exactly as they
are. This task changes only the `com.obytes.*` app IDs below.
...
```bash
grep -rn "com\.obytes\|prebuild:staging\|build:staging" .github/ package.json && echo "LEFTOVERS FOUND" || echo "clean"
pnpm type-check
```

Expected: `clean`; type-check green.

- [ ] **Step 7: Commit**

```bash
git add package.json .github/workflows/
git commit -m 'ci: point E2E at com.iapp app ids, remove staging ghosts'
```

...
- [ ] **Step 1: Rename with history preserved — commit the rename ALONE**

Rename detection only fires when the blob is similar; renaming and
rewriting in one commit records an unrelated delete+add and breaks
`git log --follow`. So the pure rename is its own commit:

```bash
git mv claude.md CLAUDE.md
git commit -m 'docs: rename claude.md to CLAUDE.md for case-sensitive loaders'
```
...
- [ ] **Step 3: Verify and commit the rewrite (second commit)**
...
Expected: `CLAUDE.md` listed; lint green. Note: eslint's
`unicorn/filename-case` gates markdown filenames — add `CLAUDE.md` to the
existing ignore array in `eslint.config.mjs` (alongside `README.md`).
Afterwards `git log --follow CLAUDE.md` must traverse into `claude.md`'s
prior commits.
